...
{
    /// <summary>
    /// Potansiyel eşleşmeleri canlandırmak için yardımcı yöntem
    /// </summary>
    ...
    /// <summary>
    /// Bir şeklin diğerinin yanında olup olmadığını kontrol eder
    ...
    /// <summary>
    /// Dikey ve yatay olarak olası eşleşmeleri kontrol eder
    /// </summary>
    /// <returns></returns>
    public static IEnumerable<GameObject> GetPotentialMatches(ShapesArray shapes)
    {
        ...
        for (int row = 0; row < Constants.Rows; row++)
        {
            for (int column = 0; column < Constants.Columns; column++)
            {
                ...
                //= 3 eşleşmemiz varsa, rastgele bir tane döndür
                if (matches.Count >= 3)
                    return matches[UnityEngine.Random.Range(0, matches.Count - 1)];

                //3'ten az eşleşmemiz varsa rastgele bir tane döndür
